...
- `Settings…`: ログイン時の自動起動、自動選択または特定のスピーカー指定、Customized EQの6バンド調整（−3〜+3 dB、0.5 dB刻み）
- `Quit`
...
```sh
make notarize-macos VERSION=0.1.5
```

別のnotary profileまたは公開先を使う場合は、`NOTARY_PROFILE`、`CASK_RELEASE_BASE_URL`、`CASK_HOMEPAGE`を`make`へ指定できます。公開URLは直下へ`v0.1.5/EdfController-0.1.5.zip`を配置できる形式にします。
